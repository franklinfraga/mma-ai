...
class AgeCalculator(BaseCalculator):
    """
    Calculator for computing fighter age at the time of each fight.
    
    This calculator calculates the age of each fighter at the time of their fights
    based on their date of birth and the event date.
    """
    ...
    def __init__(self, conn_or_context, calculator_type='single_table'):
        ...
        self.features = None
        self.feature_type = 'age'  # Used for SQL template identifier
        
        # Required for single_table calculator type
        self.table_name = 'fight_stats_fe'
        
        # Configure column filtering patterns
        self.add_include_pattern('age')  # Only process age-related columns
    ...
    def get_features(self, table_name: str = None) -> List[str]:
        """
        Get the age feature column
        
        Args:
            table_name: Optional table name (defaults to self.table_name)
            
        Returns:
            List containing the 'age' column
        """
        # We have a single feature column for age
        return ['age']
    
    def calculate(self, table_name: str = None, columns: List[str] = None) -> Dict[str, Any]:
        """
        Calculate fighter age at the time of each fight
        ...
        return {'status': 'success', 'message': 'Age calculation prepared'}
        
    def save(self, table_name: str = None, result_df: pd.DataFrame = None) -> pd.DataFrame:
        """
        Save calculated age values to the database
        ...
        self._ensure_columns_exist({'age': 'DOUBLE PRECISION'})
        ...
        if hasattr(self.context, 'sql_manager') and self.context.sql_manager:
            ...
            self.execute_raw_sql('''
                WITH age_calc AS (
                    ...
                    JOIN features.fighter_mapping m ON f.fighter_id = m.fighter_id
    ...
    def calculate_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> str:
        """
        Generate SQL for calculating age features
        ...
        """
        # We use a WITH clause to calculate ages and weight class averages
        sql = f'''
            WITH age_calc AS (
                SELECT 
                    f.fight_id,
            ...
            FROM {self.schema}.{table_name} f
            LEFT JOIN age_calc ac ON 
                f.fight_id = ac.fight_id AND 
                f.fighter_id = ac.fighter_id
            LEFT JOIN {self.schema}.fight_mapping fm ON 
                f.fight_id = fm.fight_id
            LEFT JOIN weight_class_avgs wca ON 
                fm.weightclass = wca.weightclass
        '''
        return sql
    
    def execute_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> pd.DataFrame:
        """
        Execute age calculation for a specific table and return results
    ...
    def save_for_table(self, table_name: str) -> pd.DataFrame:
        """
        Save age calculation results to the specified table
    ...
    def run(self):
        """
        Run the age calculator (for backward compatibility)
        """
