...
class FirstTimeSdevCalculator(BaseCalculator):
    """
    Precompute a table of weightclass-level stat standard deviations for first-time fighters.

    For each table in self.stat_table, gather all the <stat> values from each first-time fighter's row data. 
    Calculate the standard deviation of all those values and alias it as <stat>_wc_sdev.
    Gather the data from all first-time fighters' rows, then limit it to only fights from 2014-2023 before doing 
    the standard deviation.

    The reuslts are stored in a table:
        features.<table>_first_time_wc_sdev_stats

    with each computed column named as <stat>_wc_sdev
    """
    ...
    def __init__(self, conn_or_context, include_patterns: Set[str] = set(), exclude_patterns: Set[str] = set()):
        ...
        self.table_suffix = '_first_time_sdev_stats'  # Updated table suffix
        ...
        # Precompute the fallback statistics immediately upon initialization
        try:
            self.precompute_first_time_stddev_stats_for_all_tables()
        except Exception as e:
            self.logger.error(f"Error precomputing stats: {str(e)}")
            # Don't re-raise to allow for partial initialization
    ...
    def _create_first_time_sdev_table(self, table_name: str, columns: List[str]) -> None:
        """Create a new table for storing first-time fighter stat standard deviations."""
        try:
            ...
            # Build STDDEV expressions for each stat column with the correct suffix
            stddev_selects = [
            ...
            self.logger.info(f"Using inline SQL for first-time fighter stddev calculation for {table_name}")
            create_sql = f"""
            ...
            self.logger.error(f"Error creating first time sdev table for {table_name}: {str(e)}")
            self.conn.rollback()
            raise

    def _validate_sdev_stats(self, table_name: str) -> None:
        """Validate the computed stat standard deviation statistics."""
        try:
            ...
            # Check for negative values (which shouldn't happen with stddev)
            for col in stats_df.columns:
                if col != 'weightclass':
                    if stats_df[col].min() < 0:
                        self.logger.warning(f"Negative standard deviation in {table_name}.{col}")
    ...
    def precompute_first_time_stddev_stats_for_all_tables(self) -> Dict[str, pd.DataFrame]:
        """
        Precompute stat standard deviation statistics for all feature tables.
        ...
        self.logger.info(f"Starting first-time fighter stddev calculation for {total_tables} tables")
        print(f"\n=== Starting first-time fighter stddev calculation for {total_tables} tables ===")
        
        # For each stat table, precompute standard deviations of stats
        for i, table_name in enumerate(tables_to_process, 1):
            try:
                ...
                self.logger.info(f"[{i}/{total_tables}] Computing first-time fighter stddev stats for {table_name} with {len(filtered_columns)} columns")
                print(f"  └─ [{i}/{total_tables}] Processing {table_name} with {len(filtered_columns)} columns")
                
                # Create the standard deviation table
                self._create_first_time_sdev_table(table_name, filtered_columns)
        ...
        print(f"=== First-time fighter stddev calculation completed ===\n")
        self.logger.info(f"Completed first-time fighter stddev calculation for {len(results)} tables")
        return results
    ...
    def run(self, parallel: bool = False, max_workers: int = 4) -> Dict[str, pd.DataFrame]:
        """
        Run the first time stat standard deviation calculator for all tables.
        ...
        """
        return self.precompute_first_time_stddev_stats_for_all_tables()