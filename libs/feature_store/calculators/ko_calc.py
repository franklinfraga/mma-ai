...
class KOCalculator(BaseCalculator):
    """
    Calculator for computing knockout (KO) features.
    
    This calculator determines whether a fighter won by knockout
    based on the method of victory from the fight_mapping table.
    It also tracks which round the KO occurred in.
    """
    ...
    def __init__(self, conn_or_context, calculator_type='single_table'):
        ...
        self.features = None
        self.feature_type = 'ko'  # Used for SQL template identifier
        
        # Required for single_table calculator type
        self.table_name = 'fight_stats_fe'
        
        # Configure column filtering patterns
        self.add_include_pattern('ko')  # Only process KO-related columns
    ...
    def get_features(self, table_name: str = None) -> List[str]:
        ...
        return ['ko', 'ko_rd1', 'ko_rd2', 'ko_rd3', 'ko_rd4', 'ko_rd5']

    def calculate(self, table_name: str = None, columns: List[str] = None) -> Dict[str, Any]:
        """
        Calculate knockout (KO) feature based on method of victory and track which round the KO occurred in
        ...
        if missing_columns:
            ...
            for col in ['ko', 'ko_rd1', 'ko_rd2', 'ko_rd3', 'ko_rd4', 'ko_rd5']:
        ...
        # Initialize KO columns with 0
        self.features['ko'] = 0
        self.features['ko_rd1'] = 0
        self.features['ko_rd2'] = 0
        self.features['ko_rd3'] = 0
        self.features['ko_rd4'] = 0
        self.features['ko_rd5'] = 0
        
        # KO methods of victory
        ko_methods = [
            'KO/TKO', 'TKO - Doctor\'s Stoppage', 'KO', 'TKO', 
            'TKO - Punches', 'TKO - Retirement', 'TKO - Injury'
        ]
        
        # Set ko=1 for fighters who won by KO
        for fight_id in self.features['fight_id'].unique():
            fight_rows = self.fight_mapping[self.fight_mapping['fight_id'] == fight_id]
            
            # Get method to determine if it was a KO
            method = fight_rows['method'].iloc[0]
            
            # Skip if not a KO method
            if method not in ko_methods:
                continue
            ...
            # Set ko=1 for the winner
            winner_mask = (self.features['fight_id'] == fight_id) & (self.features['fighter_id'] == winner_id)
            self.features.loc[winner_mask, 'ko'] = 1
            
            # Set the appropriate round-specific ko column
            if end_round == 1:
                self.features.loc[winner_mask, 'ko_rd1'] = 1
            elif end_round == 2:
                self.features.loc[winner_mask, 'ko_rd2'] = 1
            elif end_round == 3:
                self.features.loc[winner_mask, 'ko_rd3'] = 1
            elif end_round == 4:
                self.features.loc[winner_mask, 'ko_rd4'] = 1
            elif end_round == 5:
                self.features.loc[winner_mask, 'ko_rd5'] = 1
        
        return {"status": "success", "feature_count": len(self.features), "table_name": table_name}
        
    def save(self, table_name: str = None, result_df: pd.DataFrame = None) -> pd.DataFrame:
        """
        Save the calculated KO features
    ...
    def calculate_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> str:
        """
        Generate SQL for calculating KO features (if using SQL-based approach)
        ...
        """
        # For KO calculator, we don't use SQL for calculation but rather Python logic
    ...
    def execute_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> pd.DataFrame:
        """
        Execute KO calculation for a specific table and return results
        ...
        # Create a results dataframe for the KO features
        ...
        for fight_id in result_df['fight_id'].unique():
            ...
            result_df.loc[winner_mask, 'ko'] = 1
            
            # Set the appropriate round-specific ko column
            if end_round == 1:
                result_df.loc[winner_mask, 'ko_rd1'] = 1
            elif end_round == 2:
                result_df.loc[winner_mask, 'ko_rd2'] = 1
            elif end_round == 3:
                result_df.loc[winner_mask, 'ko_rd3'] = 1
            elif end_round == 4:
                result_df.loc[winner_mask, 'ko_rd4'] = 1
            elif end_round == 5:
                result_df.loc[winner_mask, 'ko_rd5'] = 1
    ...
    def save_for_table(self, table_name: str) -> pd.DataFrame:
        """
        Save KO calculation results to the specified table
    ...
    def run(self, parallel: bool = False, max_workers: int = 4) -> Dict[str, pd.DataFrame]:
        """
        Run the KO calculator with either sequential or parallel execution
