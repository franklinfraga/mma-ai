...
class RatioCalculator(BaseCalculator):
    """
    Calculator for computing ratio features.
    
    Formula: fighter_stat / (fighter_stat + opponent_stat)
    Creates bounded ratio values between 0 and 1 for comparing fighter performance.
    """
    ...
    def __init__(self, conn_or_context, calculator_type='single_table'):
        ...
        self.features = None
        self.feature_type = 'ratio'  # Used for SQL template identifier
        
        # Exclude patterns for columns we don't want ratio for
        self.exclude_patterns = {'_id', '_sdev', '_ratio', '_acc', '_def', '_per_min', 'time_sec', 'time_sec_rd1', '_raw'}
    ...
    def get_features(self, table_name: str = None) -> List[str]:
        """
        Get features for ratio calculations.
    ...
    def calculate(self, table_name: str = None, columns: List[str] = None) -> Dict[str, Any]:
        """
        Calculate ratio metrics for specific features.
        ...
        try:
            # Generate SQL for ratio metrics
            sql = self._generate_ratio_sql(table_name)
            
            if not sql:
                return {"status": "skipped", "message": "Failed to generate SQL", "table_name": table_name}
                
            return {"status": "success", "sql": sql, "feature_count": len(self.features), "table_name": table_name}
        except Exception as e:
            self.logger.error(f"Error calculating ratio metrics: {str(e)}")
            import traceback
            self.logger.error(traceback.format_exc())
            return {"status": "error", "message": str(e), "table_name": table_name}

    def _generate_ratio_sql(self, table_name: str) -> str:
        """
        Generate SQL for calculating ratio features.
        
        Args:
            table_name: Name of the table to calculate for
            
        Returns:
            SQL string for ratio calculation
        """
        try:
            # Generate ratio column definitions
            ratio_columns = []
            for col in self.features:
                # Create ratio column that handles NULL values
                ratio_col = f"""
                CAST(
                    CASE
                        WHEN (f1.{col} + f2.{col}) = 0 THEN 0.5
                        WHEN (f1.{col} + f2.{col}) IS NULL THEN NULL
                        ELSE ROUND(f1.{col}::NUMERIC / NULLIF(f1.{col} + f2.{col}, 0)::NUMERIC, 4)
                    END
                AS FLOAT) AS {col}_ratio
                """
                ratio_columns.append(ratio_col)
                
            ratio_col_str = ", ".join(ratio_columns)
            
            # Basic SQL for fighter ratio calculations - simplified version
            sql = f"""
            ...
            ratio_calc AS (
                SELECT
                    fp.fight_id,
                    fp.fighter1_id as fighter_id,
                    {ratio_col_str}
                FROM fighter_pairs fp
                JOIN {self.schema}.{table_name} f1 ON fp.fight_id = f1.fight_id AND fp.fighter1_id = f1.fighter_id
                JOIN {self.schema}.{table_name} f2 ON fp.fight_id = f2.fight_id AND fp.fighter2_id = f2.fighter_id
                
                UNION ALL
                
                SELECT
                    fp.fight_id,
                    fp.fighter2_id as fighter_id,
                    {", ".join([f"""
                    CAST(
                        CASE
                            WHEN (f2.{col} + f1.{col}) = 0 THEN 0.5
                            WHEN (f2.{col} + f1.{col}) IS NULL THEN NULL
                            ELSE ROUND(f2.{col}::NUMERIC / NULLIF(f2.{col} + f1.{col}, 0)::NUMERIC, 4)
                        END
                    AS FLOAT) AS {col}_ratio
                    """ for col in self.features])}
                FROM fighter_pairs fp
            ...
            self.logger.error(f"Error generating ratio SQL: {str(e)}")
            import traceback
            self.logger.error(traceback.format_exc())
            return ""
    ...
    def save(self, table_name: str = None, result_df: pd.DataFrame = None) -> pd.DataFrame:
        """
        Save the calculated ratio features.
        ...
        # Generate ratio column names
        ratio_columns = [feat + '_ratio' for feat in self.features]
    ...
    def execute_sql_template(self, template_name: str, operation: str, params: Dict) -> str:
        ...
        try:
            ...
            # Ensure we're casting to numeric for ratio values
            if operation == 'calculate' and sql:
                # Check if SQL contains ratio calculations
                if '_ratio' in sql and 'CASE WHEN' in sql:
                    # Find all patterns like "CASE WHEN ... END AS col_ratio"
                    import re
                    sql = re.sub(
                        r'(CASE WHEN.*?END) AS ([a-z_]+_ratio)',
    ...
    def calculate_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> str:
        """
        Generate SQL for calculating ratio features for a specific table.
        ...
        # Use SQL template for calculation, similar to acc_calc.py
        sql = self.execute_sql_template(
            ...
            params={
                ...
                'feature_utils': self.context.feature_utils if hasattr(self.context, 'feature_utils') and self.context.feature_utils else None
            }
        )
        
        # If template execution failed, fall back to direct SQL generation
        if not sql:
            sql = self._generate_ratio_sql(table_name)
    ...
    def execute_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        Args:
            table_name: Name of the table to calculate
            columns: Optional list of columns to calculate
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Error executing SQL for {table_name}: {str(e)}")
                self.logger.debug(f"SQL that failed: {sql}")
                return pd.DataFrame()
                
        except Exception as e:
            self.logger.error(f"Error in execute_for_table for {table_name}: {str(e)}")
            return pd.DataFrame()
    ...
    def save_for_table(self, table_name: str, columns: Optional[List[str]] = None, 
        ...
        if len(ratio_columns) > 0 and not all(col in result_df.columns for col in ratio_columns):
            # Some expected columns are missing, log a warning
            missing = [col for col in ratio_columns if col not in result_df.columns]
            self.logger.warning(f"Missing columns in result_df: {missing}")
            
            # Only keep columns that exist
            ratio_columns = [col for col in ratio_columns if col in result_df.columns]
