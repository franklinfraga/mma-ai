...
class TimedecStdDevCalculator(BaseCalculator):
    """
    Calculate a time-decayed rolling standard deviation for each column, using
    an exponential decay with a configurable half-life (in years). For a fighter's 
    very first fight, fall back to the precomputed weightclass-level standard deviation.
    ...
    This version uses a lateral join so that the current fight's event_date is "frozen"
    for computing the sums over all past fights.
    """
    ...
    def __init__(self, context_or_conn, decay_rate_years: float = 1.5,
        ...
        """
        Initialize the time-decayed standard deviation calculator.
    ...
    def calculate_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> str:
        """
        Generate SQL for calculating time-decayed standard deviations for a table.
        ...
        try:
            ...
            # Filter out columns already ending with _dec_sdev to avoid re-processing
            ...
            # For each column we will compute its weighted sum and weighted sum-of-squares.
            sum_wx_exprs = []
            sum_wx2_exprs = []
            for col in calc_columns:
                sum_wx_exprs.append(
                    f"SUM(b2.{col} * EXP(-{self.decay_rate} * ((b.event_date - b2.event_date)::INTEGER / 365.25))) AS sum_wx_{col}"
                )
                sum_wx2_exprs.append(
                    f"SUM((b2.{col})^2 * EXP(-{self.decay_rate} * ((b.event_date - b2.event_date)::INTEGER / 365.25))) AS sum_wx2_{col}"
                )
            
            sum_wx_exprs_str = ",\n                ".join(sum_wx_exprs)
            sum_wx2_exprs_str = ",\n                ".join(sum_wx2_exprs)
            
            # For each column, build the case statement for std dev calculation
            col_calcs = []
            for col in calc_columns:
                # Use fallback from features.<table>_first_time_sdev_stats;
                # the fallback column is named <col>_wc_sdev.
                fallback_col = f"{col}_wc_sdev"
                col_calcs.append(f"""
                CASE 
                    WHEN agg.count_fights <= 1 THEN COALESCE(ftss.{fallback_col}, 0)
                    WHEN agg.sum_w = 0 THEN 0
                    ELSE SQRT(
                        GREATEST(
                            (agg.sum_wx2_{col} / agg.sum_w) - ((agg.sum_wx_{col} * agg.sum_wx_{col}) / (agg.sum_w * agg.sum_w)),
                            0
                        )
                    )
            ...
            template_params = {
                ...
                'sum_wx_exprs_str': sum_wx_exprs_str,
                'sum_wx2_exprs_str': sum_wx2_exprs_str,
                'col_calcs_str': col_calcs_str
            }
            
            # Render template
            sql = self.execute_sql_template(
                'time_decayed_sdev',  # Template directory
    ...
    def execute_for_table(self, table_name: str, columns: Optional[List[str]] = None) -> pd.DataFrame:
        """
        Execute time-decayed standard deviation calculation for the given table.
        
        Args:
            table_name: The name of the table to calculate time-decayed standard deviations for
            columns: The list of columns to calculate time-decayed standard deviations for
            
        Returns:
            DataFrame with time-decayed standard deviation calculation results
        """
        self.logger.info(f"Executing time-decayed standard deviation calculation for {table_name}")
        ...
        try:
            ...
            # Execute SQL to calculate time-decayed standard deviations
            df = self.execute_raw_sql(sql, return_results=True)
            self.logger.info(f"Completed time-decayed standard deviation calculation for {table_name} with {len(df)} rows")
            print(f"  └─ Successfully calculated time-decayed standard deviations: {len(df)} rows")
            return df
            
        except Exception as e:
            self.logger.error(f"Error executing time-decayed standard deviation calculation for {table_name}: {str(e)}")
            print(f"  └─ Error calculating time-decayed standard deviations: {str(e)}")
            raise

    def run(self, parallel: bool = False, max_workers: int = 4, table_pattern: str = "") -> Dict[str, pd.DataFrame]:
        """
        Run the time-decayed standard deviation calculator for all tables.
        ...
        """
        self.logger.info(f"Starting time-decayed standard deviation calculation for all tables")
        print(f"\n=== Starting time-decayed standard deviation calculation ===")
        ...
        for i, table_name in enumerate(tables, 1):
            try:
                ...
                self.logger.info(f"Completed time-decayed standard deviation calculation for {table_name}")
        ...
        print(f"\n=== Completed time-decayed standard deviation calculation: {successful_tables}/{total_tables} tables successfully processed ===\n")
        return results
        
    def save_for_table(self, table_name: str, columns: List[str], df: pd.DataFrame) -> pd.DataFrame:
        """
        Save time-decayed standard deviation calculation results for a table.
        ...
        self.logger.info(f"Saving {len(df)} time-decayed standard deviation features for table: {table_name}")
        print(f"  └─ Saving {len(df)} time-decayed standard deviation features for {table_name}")
        try:
            ...
            self.logger.info(f"Successfully saved time-decayed standard deviation features for {table_name}")
            print(f"  └─ Successfully saved to database")
            return df
        except Exception as e:
            self.logger.error(f"Error saving time-decayed standard deviation features for {table_name}: {str(e)}")
