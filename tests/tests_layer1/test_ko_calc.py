...
def test_ko_calculator_initialization():
    """Test KOCalculator initialization with different calculator types"""
    mock_conn = MagicMock()
    
    # Test default initialization
    calculator = KOCalculator(mock_conn)
    assert calculator.calculator_type == 'single_table'
    
    # Test with explicit calculator_type
    calculator = KOCalculator(mock_conn, calculator_type='multi_table')
    ...
    calculator = KOCalculator(mock_context, calculator_type='cross_table')
    assert calculator.calculator_type == 'cross_table'
    
    # Test column filter initialization
    calculator = KOCalculator(mock_conn)
    # Should have include pattern for 'ko'
    assert any(pattern == 'ko' for pattern in calculator.include_patterns)
    
    # Test table_name is set
    assert calculator.table_name == 'fight_stats_fe'


def test_ko_calculator_with_context():
    """Test KOCalculator with CalculatorContext and mock data"""
    # Create mock data
    mock_data = {
    ...
    with patch('libs.feature_store.calculator_context.FeatureUtils'):
        with patch('libs.feature_store.calculator_context.SQLTemplateManager'):
            ...
            with patch.object(pd, 'read_sql', return_value=mock_joined_data):
                ...
                calculator = KOCalculator(context)
                ...
                assert 'ko' in calculator.features.columns
                assert 'ko_rd1' in calculator.features.columns
                assert 'ko_rd2' in calculator.features.columns
                assert 'ko_rd3' in calculator.features.columns
                assert 'ko_rd4' in calculator.features.columns
                assert 'ko_rd5' in calculator.features.columns
                
                # Check that fighter 1 got a KO in fight 101
                ...
                # Check that fighter 1 got a KO in round 1
                assert calculator.features.loc[
                    (calculator.features['fight_id'] == 101) & 
                    (calculator.features['fighter_id'] == 1), 
                    'ko_rd1'
                ...
                # Check that fighter 3 did not get a KO in fight 102 (Decision)
                assert calculator.features.loc[
                ...
                # Check that fighter 6 did not get a KO in fight 103 (Submission)
                assert calculator.features.loc[
...
def test_ko_calculator_with_sql_template():
    """Test KOCalculator with SQL template manager"""
    # Create mock data
    mock_data = {
        'fight_mapping': pd.DataFrame({
            'fight_id': [101, 102, 103],
...
def test_ko_calculator_sequential_execution():
    """Test KOCalculator sequential execution"""
...
def test_ko_calculator_parallel_execution():
    """Test KOCalculator parallel execution"""
    # Create mock connection
    mock_conn = MagicMock()
...
def test_ko_calculator_full_integration():
    """Full integration test for KOCalculator with mocked dependencies"""
    # Create mock data
    mock_data = {
        'fight_mapping': pd.DataFrame({
            'fight_id': [101, 102, 103],
...
def test_ko_calculator_column_filtering():
    """Test KOCalculator column filtering functionality"""
    mock_conn = MagicMock()
    calculator = KOCalculator(mock_conn)
    
    # Test include pattern
    assert calculator.should_process_column('ko')
    assert calculator.should_process_column('ko_rd1')
    
    # Test exclude pattern (add one for testing)
    calculator.add_exclude_pattern('ko_rd2')
    assert not calculator.should_process_column('ko_rd2')
    
    # Test with list of columns
    all_columns = ['ko', 'ko_rd1', 'ko_rd2', 'ko_rd3', 'other_column']
    filtered = calculator.filter_columns(all_columns)
    assert 'ko' in filtered
    assert 'ko_rd1' in filtered
    assert 'ko_rd2' not in filtered
